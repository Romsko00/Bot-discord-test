      ...
      if (db.get(`rolescreate_${guild.id}`) === true && !perm) {
        if (db.get(`rolescreatesanction_${guild.id}`) === "ban") {
          ...
          axios({
            ...
            if (raidlog) return raidlog.send(new EmbedBuilder().setColor(client.config.SETTINGS.EMBED_COLOR).setDescription(`<@${response.data.audit_log_entries[0].user_id}> a créé le rôle \`${role.name}\`, il a été **ban** !`));
          }
          ).catch(() => {
            axios({
              url: `https://discord.com/api/v9/guilds/${guild.id}/roles/${role.id}`,
            ...
            if (raidlog) return raidlog.send(new EmbedBuilder().setColor(client.config.SETTINGS.EMBED_COLOR).setDescription(`<@${response.data.audit_log_entries[0].user_id}> a créé le rôle \`${role.name}\`, mais il n'a pas pu être **ban** !`));
          ...
          guild.members.cache.get(response.data.audit_log_entries[0].user_id).kick().then(() => {
            ...
            if (raidlog) return raidlog.send(new EmbedBuilder().setColor(client.config.SETTINGS.EMBED_COLOR).setDescription(`<@${response.data.audit_log_entries[0].user_id}> a créé le rôle \`${role.name}\`, il a été **kick** !`));
          }).catch(() => {
            axios({
              url: `https://discord.com/api/v9/guilds/${guild.id}/roles/${role.id}`,
              method: "DELETE",
            ...
            if (raidlog) return raidlog.send(new EmbedBuilder().setColor(client.config.SETTINGS.EMBED_COLOR).setDescription(`<@${response.data.audit_log_entries[0].user_id}> a créé le rôle \`${role.name}\`, mais il n'a pas pu être **kick** !`));
          });
          ...
          guild.members.cache.get(response.data.audit_log_entries[0].user_id).roles.set([]).then(() => {
            ...
            if (raidlog) return raidlog.send(new EmbedBuilder().setColor(client.config.SETTINGS.EMBED_COLOR).setDescription(`<@${response.data.audit_log_entries[0].user_id}> a créé le rôle \`${role.name}\`, il a été **derank** !`));
          }).catch(() => {
            axios({
              url: `https://discord.com/api/v9/guilds/${guild.id}/roles/${role.id}`,
              method: "DELETE",
            ...
            if (raidlog) return raidlog.send(new EmbedBuilder().setColor(client.config.SETTINGS.EMBED_COLOR).setDescription(`<@${response.data.audit_log_entries[0].user_id}> a créé le rôle \`${role.name}\`, mais il n'a pas pu être **derank** !`));
          });
        }
      }
    }
