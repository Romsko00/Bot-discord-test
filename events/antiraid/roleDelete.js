...
module.exports = (client, role) => {
  ...
  axios.get(`https://discord.com/api/v9/guilds/${guild.id}/audit-logs?limit=1&action_type=32`, {
    headers: {
      ...
      if (db.get(`rolesdel_${guild.id}`) === true && !perm) {
        if (db.get(`rolesdelsanction_${guild.id}`) === "ban") {
          ...
          axios({
            ...
            if (raidlog) return raidlog.send(new EmbedBuilder().setColor(client.config.SETTINGS.EMBED_COLOR).setDescription(`<@${response.data.audit_log_entries[0].user_id}> a supprimé le rôle \`${role.name}\`, il a été **ban** !`));
          }
          ).catch(() => {
            role.guild.roles.create({
              data: {
            ...
            if (raidlog) return raidlog.send(new EmbedBuilder().setColor(client.config.SETTINGS.EMBED_COLOR).setDescription(`<@${response.data.audit_log_entries[0].user_id}> a supprimé le rôle \`${role.name}\`, mais il n'a pas pu être **ban** !`));
          ...
          guild.members.cache.get(response.data.audit_log_entries[0].user_id).kick().then(() => {
            ...
            if (raidlog) return raidlog.send(new EmbedBuilder().setColor(client.config.SETTINGS.EMBED_COLOR).setDescription(`<@${response.data.audit_log_entries[0].user_id}> a supprimé le rôle \`${role.name}\`, il a été **kick** !`));
          }).catch(() => {
            role.guild.roles.create({
              data: {
                name: role.name,
            ...
            if (raidlog) return raidlog.send(new EmbedBuilder().setColor(client.config.SETTINGS.EMBED_COLOR).setDescription(`<@${response.data.audit_log_entries[0].user_id}> a supprimé le rôle \`${role.name}\`, mais il n'a pas pu être **kick** !`));
          });
          ...
          guild.members.cache.get(response.data.audit_log_entries[0].user_id).roles.set([]).then(() => {
            ...
            if (raidlog) return raidlog.send(new EmbedBuilder().setColor(client.config.SETTINGS.EMBED_COLOR).setDescription(`<@${response.data.audit_log_entries[0].user_id}> a supprimé le rôle \`${role.name}\`, il a été **derank** !`));
          }).catch(() => {
            role.guild.roles.create({
              data: {
                name: role.name,
            ...
            if (raidlog) return raidlog.send(new EmbedBuilder().setColor(client.config.SETTINGS.EMBED_COLOR).setDescription(`<@${response.data.audit_log_entries[0].user_id}> a supprimé le rôle \`${role.name}\`, mais il n'a pas pu être **derank** !`));
          });
        }
      }
    }
